...
메인 Claude가 **오케스트레이터**, `.claude/agents/`의 서브에이전트가 **워커**. 메인이 결과를 수집·검토하고 다음 단계를 결정한다.

| 에이전트 | 역할 | 사용 위치 | spawn 조건 |
|---------|------|----------|-----------|
| `page-md-generator` | OCR txt → page MD | analyze-invoice Phase 1 | 5페이지 이상 시 3장씩 병렬 |
| `retailer-mapper` | OCR 거래처명 → 소매처코드 | analyze-invoice Phase 3 | product-mapper와 동시 spawn |
| `product-mapper` | OCR 제품명 → 제품코드 | analyze-invoice Phase 3 | retailer-mapper와 동시 spawn |
...
기본값은 OFF — 레거시 경로가 항상 fallback으로 보장됩니다.
...
환경변수가 없거나 `false`이면 기존 CSV 주입 방식(legacy)이 사용됩니다.
...
1. ⏭ form 02·03·05 `form_definitions/form_XX.md` 작성 (업무규칙 수령 후)
...
2. "form_XX 동기화해줘"  (sync-form-config 스킬)
   → Schema 검증 + 회귀 테스트 자동 실행
...
이상 시: `cp config/form_types.json.bak config/form_types.json` 으로 롤백

---

**Last Updated:** 2026-06-05 (Phase3 Tool Use 운영 적용 완료 — retailer/product/dist 전체 검증, Global Semaphore, FastAPI token DB 확인, 645 tests passed)
