  ...
  "cached_retailers_needing_dist": [
    {
      ...
      "jisho_values": ["R営業東北"]   // 있을 때만 포함. 해당 거래처 items에서 추출한 그룹 식별 필드 값
    },
    ...
  ],
  "uncached_products": [
...
1. 양식 정의의 거래처명 구조를 먼저 확인한다.
   ...
   - 예: 다른 양식은 retail_user.csv → domae_retail_2.csv 순으로 이름 검색
...
1. **jisho_values가 있으면 먼저 참조한다** (양식 정의의 "판매처 결정 규칙" 섹션 참조)
   - jisho는 해당 거래처 items에서 추출한 그룹 식별 필드 값(담당 영업부문·지역 등)이다
   - 양식 정의에 jisho 패턴↔판매처 대응표가 있으면 그것과 대조해 확정한다
   - 일치하는 규칙이 있으면: confidence=high, 추론 근거를 basis에 기술
2. **jisho_values가 없거나 jisho로 특정 불가 시**: issuer 정보(name, tel 등)를 참고한다
  ...
  "dist_only": [
    {
      ...
      "dist_code": "D002",
      "confidence": "high",
      "basis": "issuer.name '国分グループ本社株式会社 東日本' → 판매처 '東日本支店' 부분 일치"
    }
  ],
  "products": [
    {
  ...
  "dist_only": [
    {
      ...
      "dist_code": "D205",
      "confidence": "high",
      "basis": "jisho_values ['R営業東北'] → 양식 정의 판매처 결정 규칙 대조 → D205 확정"
    }
  ],
  "products": [
    {
