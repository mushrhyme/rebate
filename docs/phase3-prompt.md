...
1. issuer 정보(name, tel 등)를 참고해 candidates 중 가장 적합한 판매처를 추론한다.
   - 발행처명·지점명·담당부서·연락처(TEL) 등을 판매처명과 대조
   - 완벽 일치 불필요 — 문맥상 가장 합리적인 판매처를 선택
   - 추론 근거를 basis 필드에 기술
2. 확신: confidence=high, dist_code 확정 → dist_only[] 출력
3. 불확실: status=NEEDS_CONFIRMATION, candidates 포함 → dist_only[] 출력
  ...
  "dist_only": [
    {
      ...
      "dist_code": "D205",
      "confidence": "high",
      "basis": "issuer.name '国分グループ本社株式会社 東日本' → candidates 중 '東日本第一グループ' 부분 일치로 D205 선택"
    }
  ],
  "products": [
    {
