...
## 消費税率 규칙 — config/tax_rules.json

タイプ명→세율 매핑은 코드에 하드코딩하지 않고 `config/tax_rules.json`을 단일 출처로 한다.

| 키 | 의미 |
| --- | --- |
| `zero_rated_types` | 세율 0%로 처리할 タイプ명 목록 (현재: `非課税`, `ロットアウト`) |
| `rate_keywords` | タイプ명에 포함된 키워드 → 세율 (예: `"10%"` → 0.10) |
| `default_rate` | 위에 해당 없을 때 기본 세율 (0.08) |
| `bracket_rates` | 8%/10% 집계 버킷 세율 (`scripts/phase4_calc.py` 得意先별 소계 표시용) |

읽는 곳: `backend/pipeline/phase4.py` `_type_tax_rate()` (mtime 캐시 — 수정 시 재시작 불필요),
`scripts/phase4_calc.py` (실행 시 로드). 파일이 없으면 **명시적 오류** — 숨은 기본값 없음.
회귀 고정: `tests/unit/test_output_format_contract.py`.

**Last Updated:** 2026-06-12 (세율 규칙 config 분리)
