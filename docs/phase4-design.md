...
1. **Python (`scripts/phase4_calc.py`)** — cover/summary `totals`(Phase 2 JSON)와 detail 합계를 수치 비교. 불일치 시 `ok: false` + diff 기록.
2. **Claude (`backend/pipeline/phase4.py`)** — 위 Python 결과를 독립적으로 재검증하고 phase4_output.json의 `xv[]`를 덮어씀. Claude가 최종 교차검증 결과를 확정한다.
