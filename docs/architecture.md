...
| 룰 단일 진실 소스 (2026-06-17) | **Literate config — form_XX.md의 `[config]` 정본 블록이 유일 소스, `form_types.json`은 거기서 빌드되는 생성물** | ① 현 이중 소스 유지(MD 산문 ↔ JSON, sync 스킬이 자연어→JSON 추론) ② JSON 정본·MD를 생성 | "룰 위치 = 이중" 행이 만든 고질병 해소. 같은 사실이 두 파일에 중복 → 드리프트·sync 누락·배포 유실(form_04 사고)의 근원. 정본을 한 파일·한 블록으로 모으면 *깨지기 쉬운 산문→구조 추론*이 표준 경로에서 사라지고(저작 시 1회만), 한쪽만 유실되는 사고가 구조적으로 불가능. 빌드 가드가 `build(MD블록)==JSON` 동치를 CI에서 보증. 설계: [literate-config-migration.md](literate-config-migration.md) |
...
- ~~**이중 룰 표기 정합성**~~ — ✅ 결정됨(2026-06-17): Literate config로 정본을 form_XX.md `[config]` 블록 하나에 통일. form_types.json은 생성물, 빌드 가드가 동치 보증. §7 의사결정 기록·[literate-config-migration.md](literate-config-migration.md) 참조.
- **공식 단일 진실 소스 (프론트·백엔드)** — form_types.json 공식을 프론트(JS)·백엔드(Python) 양쪽이 읽도록 설계. 사용자 条件2 입력 → 프론트 실시간 NET 계산, [저장] 클릭 → 백엔드 Phase 4 재실행 → DB 갱신. (literate config 전환 후 양쪽은 *생성물* form_types.json을 읽으면 됨 — 정본은 MD 블록.)
...
| 연산 레지스트리 P3 구현 (2026-06-17) | 축C(조건부 override, S6 모양): 판매처 1:N 모호 케이스를 양식 선언 규칙으로 **결정적** 확정. `backend/pipeline/dist_overrides.py` 신규(resolve_dist_override — when 정확일치 술어 + pick_candidate_name_contains/dist_code 액션, 유일매치만 확정·모호시 폴백). `get_dist_overrides(form_id)`로 form_types.json `dist_overrides` 로드, `build_dist_resolution_from_cache` 1:N 분기에 단일 훅. form_04 실재 규칙(jisho=CVS営業部 → 広域リテール 판매처) config화. schema `DistOverride` 정의 | jisho→특정 판매처 규칙이 form md 산문 + dist 1:N **LLM 프롬프트**에만 → 비결정적 | 비결정(LLM 1:N)→결정(config 규칙) 개선. 기본 OFF(dist_overrides 미선언/매칭실패/모호 → None → 기존 LLM 경로 폴백)이라 form_01 등 무영향. 골든·안전 테스트 9개(override 7 + 통합 2) + 전체 741 그린(유일 실패=무관 기존 smoke). "신라면→000"과 동일 모양 — 이후 "만약 X면 Y" 류 dist 규칙은 config 한 줄(T1). 진단(/diagnose)으로 form_04에 이미 실재하던 S6 규칙을 골든으로 확보해 추측 위험 제거 |

**Last Updated:** 2026-06-17 (연산 레지스트리 P1·P2·P3 — 집계/분해 전략 + dist 키 단일출처 + 조건부 override)
