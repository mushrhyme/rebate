...
**DSL 허용 연산**: 산술 `+` `-` `*` `/` `()`, 비교 `<` `<=` `>` `>=` `==` `!=`, 논리 `and` `or` `not`,
조건식 `A if 조건 else B`, 숫자 리터럴, 변수명. 조건식은 택한 가지만 평가하므로
`(a / b) if b != 0 else 0`처럼 0나누기 가드로 쓸 수 있다.  
**DSL 금지**: 함수 호출, 속성 접근, 문자열 — 임의 코드 실행 영구 차단(`_safe_eval`)
